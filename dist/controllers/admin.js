...
exports.updateOrderStatus = exports.getAllOrders = exports.getAnOrder = exports.adminLogin = exports.adminSignUp = exports.postMenu = undefined;
...
var adminSignUp = async function adminSignUp(req, res) {
  ...
  try {
    ...
    var token = _jsonwebtoken2.default.sign({ admin: result.rows[0] }, process.env.JWT_SECRET_ADMIN);
    res.status(200).send({
      success: true, message: 'admin account successfully created!....', details: result.rows[0], token: token
    });
  } catch (err) {
...
var adminLogin = async function adminLogin(req, res) {
  ...
  try {
    ...
    var token = _jsonwebtoken2.default.sign({ admin: rows[0] }, process.env.JWT_SECRET_ADMIN);
    res.status(200).send({
      success: true, message: 'admin successfully logged In!....', details: rows[0], token: token
    });
  } catch (err) {
    console.log(err.stack);
  } finally {
...
var getAnOrder = async function getAnOrder(req, res) {
  ...
  try {
    _jsonwebtoken2.default.verify(req.token, process.env.JWT_SECRET);
  } catch (err) {
